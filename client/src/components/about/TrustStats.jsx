export function TrustStats({ stats }) {
  return (
    <section className="bg-[#fffaf6] px-5 py-12 text-[#1F1F1F]" aria-label="KareBraids trust statistics">
      <div className="mx-auto grid max-w-7xl gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {stats.map((stat) => (
          <article className="rounded-[1.25rem] border border-[#E6DED8] bg-[#F5F1EE] p-6 text-center" key={`${stat.value}-${stat.label}`}>
