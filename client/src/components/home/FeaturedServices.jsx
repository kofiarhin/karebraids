...
export function FeaturedServices() {
  ...
  return (
    <section className="featured-services-section" aria-labelledby="featured-services-title">
      ...
      {featuredServices.length > 0 ? <div className="featured-services-grid">{featuredServices.map((service, index) => <article className="featured-service-card" data-reveal key={service.id} style={{ '--index': index + 1 }}><img alt={service.primaryImage?.alt || `${service.name} preview`} loading="lazy" src={getServicePreviewImage(service)} /><div><p>Starting at {formatPrice(service)} · {service.durationLabel}</p><h3>{service.name}</h3><Link aria-label={`Browse ${service.name} gallery`} className="text-link" to={`/gallery?service=${service.slug || service.id}`}>View Gallery</Link></div></article>)}</div> : null}
