...
export function GalleryFeature() {
  ...
  return (
    <section className="gallery-feature-section" aria-labelledby="gallery-feature-title">
      ...
      {galleryPreviewItems.length > 0 ? <div className="gallery-feature-grid" aria-label="Featured braiding gallery preview">{galleryPreviewItems.map((item, index) => <Link className="gallery-feature-card" data-reveal key={item.id} style={{ '--index': index + 1 }} to="/gallery"><img alt={item.alt} loading="lazy" src={getGalleryImageSrc(item)} /><span className="gallery-feature-overlay" aria-hidden="true" /><span className="gallery-feature-caption"><strong>{item.title}</strong><small>Representative image</small></span></Link>)}</div> : null}
